...
impl WhisperConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> Whisper<B> {
        ...
        assert!(n_audio_state == n_text_state,);
...
impl<B: Backend> TextDecoder<B> {
    fn forward(&self, x: Tensor<B, 2, Int>, xa: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        assert!(seq_len <= self.n_text_ctx,);
...
impl AudioEncoderConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> AudioEncoder<B> {
        ...
        let positional_embedding: Tensor<B, 2, Float> = Tensor::random(
            [self.n_audio_ctx, self.n_audio_state],
            Distribution::Normal(0.0, 1.0),
            device,
        );
        let positional_embedding = Param::from_tensor(positional_embedding);
...
pub fn attn_decoder_mask<B: Backend>(seq_length: usize, device: &B::Device) -> Tensor<B, 2> {
    ...
    for i in 0..(seq_length - 1) {
        let values =
            Tensor::<B, 2>::zeros([1, seq_length - (i + 1)], device).add_scalar(f32::NEG_INFINITY);
